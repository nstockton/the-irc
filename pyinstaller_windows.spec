...
# TheIRC Modules:
from theirc import __version__ as PROGRAM_VERSION  # NOQA: N812


APP_NAME: str = "TheIRC"
APP_AUTHOR: str = "Nick Stockton"
APP_VERSION_MATCH: re.Match[str] | None = re.search(
	r"^[vV]?(?P<version>\d+\.\d+\.\d+)(?P<version_type>.*)$", PROGRAM_VERSION.strip()
)
APP_VERSION: str
...
RUN_FILE_PATH: Path = Path("run_theirc.py")
IS_GIT_TAG: bool = not APP_VERSION_TYPE
...
APP_DEST_NAME: str = f"{APP_NAME}_V{APP_VERSION}{APP_VERSION_TYPE}" if IS_GIT_TAG else APP_NAME
APP_DEST_PATH: Path = ORIG_DEST_PATH.parent / APP_DEST_NAME.replace("-", "_").replace(" ", "_")
ZIP_FILE_PATH: Path = APP_DEST_PATH.with_name(f"{APP_DEST_PATH.name}.zip")
VERSION_FILE_PATH: Path = Path(tempfile.gettempdir()).expanduser().resolve() / "theirc_version.ignore"
      ...
      [
        StringTable(
          ...
          [
            ...
            StringStruct(u'FileDescription', u'{APP_NAME} V{APP_VERSION}{APP_VERSION_TYPE}'),
...
include_files: list[tuple[list[Path], Path]] = [
	...
	(
		list((APP_DEST_PATH.parent / "src" / "theirc_data" / "sounds").glob("*.wav")),
		Path("theirc_data/sounds"),
	),
]
...
# Use a fixed mtime for reproducible ZIP archives.
# ZIP format stores file times in MS-DOS 16-bit format as *local wall time*
# (not UTC). The earliest valid timestamp is 1980-01-01 00:00:00; anything
# earlier causes Python's zipfile to raise ValueError.
# We intentionally use a naive datetime (tzinfo=None) here. Calling
# .timestamp() on it produces a Unix time that, when later passed through
# time.localtime() inside zipfile, always round-trips back to the same
# wall-clock time (1980, 1, 1, 0, 0, 0), regardless of the build machine's
# timezone. This gives us bit-identical .zip files across machines in different timezones and guaranteed
# compliance with the 1980 minimum (no ValueError).
# When SOURCE_DATE_EPOCH is set we use that absolute timestamp instead.
# In that case different timezones may embed different local times in the
# ZIP (this is the normal/acceptable behavior for SOURCE_DATE_EPOCH).
...
for hashtype, func in hashes.items():
	...
	checksum_file_path.write_text(
		f"{func.hexdigest().lower()} *{ZIP_FILE_PATH.name}\n", encoding="utf-8", newline="\n"
	)
